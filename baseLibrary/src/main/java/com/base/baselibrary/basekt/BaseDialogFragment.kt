...
open class BaseDialogFragment<T : ViewBinding> : DialogFragment() {
    protected val fragment: BaseDialogFragment<T>
    get() = this
    ...
    @ColorRes
    protected var statusBarColor = android.R.color.white
    ...
    override fun onStart() {
        ...
        layoutParams.width = ScreenUtils.getAppScreenWidth() - ConvertUtils.dp2px(36f)
    }
    ...
    protected fun antiShakeClick(
        ...
        val disposable =
            view.clicks().throttleLatest(windowDuration, timeUnit).subscribe {
                activity?.runOnUiThread { onClick.run() }
            }
