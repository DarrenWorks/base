...
@JvmOverloads
fun compress(path: String, context: Context, loadingDialog: Dialog, handleResult: (String) -> kotlin.Unit, limit: Int = 3, unit: Unit = Unit.mb) {
...
@JvmOverloads
fun compress(path : List<String>, context: Context,loadingDialog: Dialog, handleResult : (List<String>) -> kotlin.Unit, limit : Int = 3, unit: Unit = Unit.mb) {
    PermissionUtils.permission(PermissionConstants.STORAGE)
        .callback(object : PermissionUtils.SimpleCallback {
            override fun onGranted() {
...
private fun compressInternal(path : String, limit : Int, unit: Unit) : String {
    LogUtils.i(File(path).length())
    val limitByte : Long = limit * unit.multiple
...
enum class Unit(val multiple: Long) {
    b(1),
    kb(1024),
    mb(1024 * 1024),
    gb(1024 * 1024 * 1024)
