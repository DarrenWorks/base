    ...
    private T parseParameterizedType(Response response, ParameterizedType type) throws Exception {
        ...
        if (rawType != LzyResponse.class) {
            ...
            if (typeArgument == Void.class) {
                ...
                final LzyResponse lzyResponse = Convert.fromJson(jsonReader, type);
                response.close();
                ...
                if (code == 200) {
                    ...
                    new Handler(Looper.getMainLooper()).post(() -> GeneralUtilsKt.showToastShort("请检查网络"));
                    throw new IllegalStateException(lzyResponse.msg);
                } else if (code == 400 || code == 1025 ) {//token失效
                    User.logOut(true);
                    new Handler(Looper.getMainLooper()).post(() -> GeneralUtilsKt.showToastShort(lzyResponse.msg));
                    throw new IllegalStateException(lzyResponse.msg);
                } else {
                    new Handler(Looper.getMainLooper()).post(new Runnable() {
                        @Override
                        public void run() {
                            GeneralUtilsKt.showToastShort(lzyResponse.msg);
                        }
                    });
                    throw new IllegalStateException(lzyResponse.msg);
                }
